...
// Normalizza una lista di email (una per riga o separate da virgola/punto e virgola):
// trim, rimuove vuoti, deduplica (case-insensitive) e ordina alfabeticamente.
function pwMailParseEmailList(text) {
  const seen = new Set();
  const out = [];
  (text || '').split(/[\n,;]+/).forEach(raw => {
    const e = raw.trim();
...
// Adatta l'altezza di una textarea al proprio contenuto (niente scrollbar interna).
function pwMailAutoGrow(el) {
  if (!el) return;
  el.style.height = 'auto';
  el.style.height = el.scrollHeight + 'px';
}

// Sincronizza l'altezza di una coppia di textarea (note/strumenti di una squadra) sulla
// più alta delle due, così le due colonne restano allineate qualunque sia il contenuto.
function pwMailAutoGrowPair(idA, idB) {
  const elA = document.getElementById(idA);
  const elB = document.getElementById(idB);
  if (!elA || !elB) return;
  elA.style.height = 'auto';
  elB.style.height = 'auto';
  const h = Math.max(elA.scrollHeight, elB.scrollHeight);
...
function pwGeneraMail() {
  ...
  // Email sempre in CC salvate, normalizzate (ordinate alfabeticamente, deduplicate)
  const emailSempreArr = pwMailParseEmailList(pwData.emailSempreIncluse || '');
  ...
  const noteSquadreHtml = squadreInfo.length > 0
    ? squadreInfo.map((s, i) => {
        ...
        const jiraHint = strumentiJiraLbl.length
          ? `<div style="font-size:10px;color:var(--accent-dark);margin-top:1px;">✓ Da griglia: ${esc(strumentiJiraLbl.join(', '))}</div>`
          : '';
        ...
        <div class="mb-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
          <details open>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; column-gap:8px; row-gap:4px; margin-top:8px;">
              ...
              <div>
                <label class="block text-[10px] text-slate-500 font-medium">🔧 Strumenti / attrezzatura <span class="text-slate-400 font-normal">(aggiuntivi, facoltativo)</span></label>
                ${jiraHint}
              </div>
                <textarea id="mail-note-sq-${i}" rows="2"
                  ...
                  style="resize:none; overflow-y:hidden; box-sizing:border-box;"
  ...
  wrap.innerHTML = `
    ...
    ">
      ...
      <div style="flex-shrink:0; padding:12px 20px; background:#f8fafc; border-bottom:1px solid #e2e8f0;">
        ...
        <details id="mail-email-sempre-details" style="margin-top:10px;">
          <summary style="font-size:11px; font-weight:600; color:#475569; cursor:pointer; user-select:none;">
            📧 Email sempre in CC
            <span id="mail-email-sempre-count" style="font-weight:400; color:#94a3b8;">(${emailSempreArr.length} indirizzo${emailSempreArr.length === 1 ? '' : 'i'} — una per riga, ordinate alfabeticamente)</span>
          </summary>
          <textarea id="mail-email-sempre" rows="3" class="pw-write-action"
            placeholder="ufficio.tecnico@eagleprojects.it&#10;capocommessa@eagleprojects.it"
            style="width:100%; margin-top:6px; border:1px solid #e2e8f0; border-radius:6px;
                   padding:6px 10px; font-size:13px; outline:none; box-sizing:border-box;
                   font-family:inherit; line-height:1.5; resize:vertical; overflow-y:hidden;"
            >${esc(emailSempreArr.join('\n'))}</textarea>
        </details>
      </div>
  ...
  // Altezza iniziale delle textarea auto-espandibili note/strumenti per squadra, allineate a coppia.
  squadreInfo.forEach((_, i) => pwMailAutoGrowPair(`mail-note-sq-${i}`, `mail-strumenti-sq-${i}`));

  // "Email sempre in CC" è collassato di default (details senza "open"): l'altezza va
  // ricalcolata solo quando viene espanso, perché un elemento dentro un <details> chiuso
  // non è renderizzato e scrollHeight risulterebbe 0.
  document.getElementById('mail-email-sempre-details').addEventListener('toggle', (e) => {
    if (e.target.open) pwMailAutoGrow(document.getElementById('mail-email-sempre'));
  });
  ...
  // Aggiorna il conteggio indirizzi mostrato nel titolo (visibile anche a details chiuso)
  function aggiornaContatoreEmailSempre(count) {
    const el = document.getElementById('mail-email-sempre-count');
    if (el) el.textContent = `(${count} indirizzo${count === 1 ? '' : 'i'} — una per riga, ordinate alfabeticamente)`;
  }
  ...
  document.getElementById('mail-email-sempre').addEventListener('input', (e) => {
    ...
    pwSave();
    pwMailAutoGrow(e.target);
    aggiornaContatoreEmailSempre(pwMailParseEmailList(e.target.value).length);
  });
  // Al blur: riordina alfabeticamente e deduplica, così la lista resta sempre "in ordine"
  // e si vede subito chi c'è/chi manca senza disturbare la digitazione in corso.
  document.getElementById('mail-email-sempre').addEventListener('blur', (e) => {
    if (!sbGuardWrite()) return;
    const lista = pwMailParseEmailList(e.target.value);
    e.target.value = lista.join('\n');
    pwData.emailSempreIncluse = e.target.value;
    pwSave();
    pwMailAutoGrow(e.target);
    aggiornaContatoreEmailSempre(lista.length);
  });
  // Note e strumenti squadra: salva in storage + rigenera anteprima, mantenendo le due
  // colonne (note/strumenti) allineate in altezza qualunque sia il contenuto.
  squadreInfo.forEach((_, i) => {
    const noteEl = document.getElementById(`mail-note-sq-${i}`);
    if (noteEl) noteEl.addEventListener('input', () => {
      salvaNoteSq();
      document.getElementById('mail-testo').value = componiTesto();
      pwMailAutoGrowPair(`mail-note-sq-${i}`, `mail-strumenti-sq-${i}`);
    });
    const strumentiEl = document.getElementById(`mail-strumenti-sq-${i}`);
  ...
  document.getElementById('mail-apri-outlook').onclick = async () => {
    ...
    const emailSempre = pwMailParseEmailList(document.getElementById('mail-email-sempre').value || '');
