...
function pwMapBuildOperatorItems(itemsAll) {
...
async function pwMapRenderResidenze(itemsAll, dayIdx) {
  const items = pwMapBuildOperatorItems(itemsAll);
  ...
  if (legendEl) {
    ...
    if (!uniq.length) {
      legendEl.innerHTML = '<div class="text-slate-400 italic" style="font-size:11px">Nessun operatore assegnato oggi</div>';
    } else {
  ...
  if (sumEl) {
    if (!items.length) {
      sumEl.innerHTML = '<div class="text-slate-400 italic" style="font-size:11px">Nessun operatore per questo giorno</div>';
    } else {
      const groupOrder = [];
      const groups = {};
      items.forEach((item, i) => {
