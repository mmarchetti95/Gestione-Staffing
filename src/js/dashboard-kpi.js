...
function showKpiModal(type) {
  ...
  /* ── ATTESTATI SCADUTI/IN SCADENZA ────────────────── */
    ...
    // raggruppa le voci per operatore, ordinando i gruppi alfabeticamente
    // (le voci dentro ogni gruppo restano ordinate per urgenza)
    const raggruppaPerOperatore = lista => {
      const gruppi = [];
      const byKey = new Map();
      lista.forEach(s => {
        const key = s.op.id || s.op.nome_esteso || s.op.nome;
        let g = byKey.get(key);
        if (!g) { g = { op: s.op, voci: [] }; byKey.set(key, g); gruppi.push(g); }
        g.voci.push(s);
      });
      gruppi.sort((a, b) => (a.op.nome_esteso||a.op.nome||'').localeCompare(b.op.nome_esteso||b.op.nome||'', 'it'));
      return gruppi;
    };

    const sezione = (lista, opts) => {
      if (!lista.length) return '';
      const gruppi = raggruppaPerOperatore(lista);
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${gruppi.map(g => `
            <div style="border:1px solid ${opts.border};border-radius:8px;background:${opts.bg};overflow:hidden;">
              <div style="padding:8px 12px;font-weight:700;color:#1e293b;font-size:13px;border-bottom:1px solid ${opts.border};">${esc(g.op.nome_esteso||g.op.nome||'')}</div>
              <div style="display:flex;flex-direction:column;">
                ${g.voci.map((s,i) => `
                  <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;padding:6px 12px;${i>0?`border-top:1px dashed ${opts.border};`:''}">
                    <span style="color:#334155;font-size:12px;">${esc(s.nome)}</span>
                    <span style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
                      <span style="color:#94a3b8;font-size:11px;white-space:nowrap;">${fmtDate(s.scad)}</span>
                      <span style="font-weight:700;color:${opts.color};font-size:11px;white-space:nowrap;min-width:78px;text-align:right;">${s.stato==='scaduto' ? ('scaduto da '+Math.abs(s.giorni)+'gg') : ('tra '+s.giorni+'gg')}</span>
                    </span>
                  </div>`).join('')}
              </div>
            </div>`).join('')}
    ...
    if (scadenze.length === 0) {
      ...
      bodyHtml = `<div style="font-size:12px;color:#64748b;margin-bottom:16px;">${scaduti.length} scaduti, ${inScadenza.length} in scadenza entro ${ATTESTATI_PREAVVISO_GG} giorni</div>
