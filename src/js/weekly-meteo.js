...
/* ----- Widget "criticità meteo settimana" (header Griglia, accanto ad Aggiorna strumenti) -----
   MVP basato solo su soglie sulle previsioni Open-Meteo già in cache (weathercode/probabilità
   di precipitazione): nessuna integrazione con bollettini Protezione Civile per ora (nessun
   feed pubblico gratuito confermato disponibile, vedi nota nel README/memoria di progetto). */
  ...
  data.forEach(bc => (bc.squadre || []).forEach(sq => {
    for (let dayIdx = 0; dayIdx < 6; dayIdx++) {
      ...
      pwSquadraCantieriGiorno(sq, dayIdx).forEach(cantiere => {
        ...
        const info = pwMeteoInfoFor(cantiere, dateISO);
        if (info) covered++;
        const severity = pwMeteoSeverityFor(info);
        if (severity) crit.push({ cantiere, dayIdx, dateISO, severity, info });
      });
    }
  }));
...
function pwOpenWeatherWeekModal() {
  ...
  const rows = crit.length
    ? crit.map(c => {
        ...
        return `<div class="pw-meteo-modal-block">
          ...
          <div class="pw-meteo-modal-info">${sevLabel} · ${pwMeteoIconFor(c.info.code)} ${Math.round(c.info.tmax)}°/${Math.round(c.info.tmin)}°${c.info.pop != null ? ' · 💧' + Math.round(c.info.pop) + '%' : ''}</div>
        </div>`;
      }).join('')
  ...
  root.innerHTML = `<div class="modal-backdrop"><div class="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 my-8 p-5">
    ...
    <p class="text-xs text-slate-500 mb-3">Basato su soglie sulle previsioni Open-Meteo (pioggia/temporali); nessuna integrazione con bollettini Protezione Civile.</p>
    <div>${rows}</div>
    <div class="flex justify-end mt-4">
      <button onclick="closeModal()" class="px-3 py-1.5 text-sm border border-slate-300 rounded">Chiudi</button>
    </div>
