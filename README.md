...
## v18.164.0
- feat: **Assistente AI — storico persistente delle domande, vista personale nel widget + pannello admin "🗨️ Storico Assistente AI"**: ogni domanda/risposta scambiata col widget viene ora salvata nella nuova tabella `ai_assistant_history` (email utente, domanda, risposta, data/ora; RLS: insert per qualunque utente autenticato, lettura per l'admin su tutte le righe o per l'utente sulle proprie — due policy separate, migrazione applicata). Nel widget, l'icona 🕘 in alto mostra a ogni utente le proprie domande passate; cliccandone una si "riapre" quella conversazione come due bubble di sola lettura (nessun input, non proseguibile) con un link per tornare all'elenco o alla chat live, che nel frattempo resta intatta. L'admin ha in più il pannello dedicato accanto a "🤖 Assistente AI" con l'elenco di tutti gli utenti (ultime 500 righe) e un filtro testo su utente/domanda/risposta. A differenza dello storico conversazionale in memoria (`_aiHistory`, usato solo come contesto per il modello e azzerato al reload), questo non torna mai al modello: serve solo per la consultazione. Nessuna Edge Function coinvolta — l'insert avviene direttamente dal client, come per il log attività.
- feat: **Gestione utenti — ricerca per email, filtro per ruolo e ordinamento per colonna**: nel pannello admin "👥 Gestione utenti", un campo di ricerca (email) e una tendina (ruolo) filtrano la lista; le intestazioni Email/Ruolo/Creato il/Ultimo accesso sono ora cliccabili per ordinare (frecce ▲/▼ per indicare colonna e verso). Filtro e ordinamento operano client-side sulla lista già caricata, senza richiamare la Edge Function `admin-users` ad ogni interazione.
...
## v18.156.0
- feat: **Nuova sezione "⚠️ Operatori con limitazioni" (Dashboard, fra Attestati e DPI) + avviso in Griglia settimanale** — importazione Excel del file "EP - Elenco dipendenti con limitazioni" (tipo visita, data visita, scadenza, fino a 2 limitazioni con nota specifica per dipendente), abbinate al pool operatori con lo stesso motore di riconoscimento nome già usato per Ferie/Attestati. La nuova sezione mostra l'elenco completo con filtri (ricerca, tipo limitazione, stato scadenza visita, solo pool) ed export Excel; un badge "⚠ limitazioni" compare anche sulla card dell'operatore nel Pool. In Pianificazione Settimanale → Griglia, assegnando a un cantiere un operatore con limitazioni compare un avviso col dettaglio (tipo visita, scadenza, limitazioni con nota) prima di confermare, con possibilità di annullare la scelta. Le limitazioni sono modificabili anche a mano dalla scheda operatore (aggiunta/rimozione voci, catalogo dei "tipo limitazione" con suggerimenti); un reimport del file sostituisce il record per chi vi compare, senza toccare chi ne è assente.

## v18.155.0
- feat: **Pianificazione Settimanale — guida contestuale col tasto destro sui 5 bottoni più delicati** — "🔄 Sincronizza da Jira" e "📄 Carica Report" (Controllo Produzione), "📥 Importa Excel" (Ferie & Permessi), "🔧 Aggiorna strumenti" (Griglia) e "🎫 Sottotask Jira" (header commessa in Griglia) mostrano al passaggio del mouse un tooltip "Click destro per la guida"; il click destro (invece del menu contestuale del browser) apre un modal scrollabile con una spiegazione breve di cosa fa il bottone. Click sinistro invariato, nessun rischio di azionare per sbaglio un'operazione (specie quelle che scrivono su Jira) mentre si cerca solo di capire cosa fa il bottone.

## v18.154.0
- feat: **Sottotask Jira da Griglia — controllo anti-duplicati per week, calcolo automatico di Stima originale/Production Weight, Target Production ereditato dal Task** — tre correzioni al flusso di creazione introdotto nelle versioni precedenti: (1) il controllo di idempotenza (Step 1/2, dryRun) ora considera anche la week corrente, non solo assignee+Task — prima un operatore con un sottotask già creato in una week precedente sotto lo stesso Task risultava erroneamente "già esistente" e non se ne poteva creare uno nuovo per la week in corso; il summary del sottotask riporta ora un suffisso "- Week N" usato per questo confronto (i sottotask creati prima di questa versione, senza il suffisso, restano coperti dal solo controllo assignee+Task); (2) nel form "campi extra" (Step 1.5), "Stima originale" e "Production Weight (%)" partono precompilati calcolati automaticamente (rispettivamente: differenza in giorni tra Start date pianificato e Data di scadenza, con ricalcolo live se si cambia una delle due date; 100% diviso il numero di sottotask selezionati nel batch) invece di valori fissi — restano comunque modificabili a mano; (3) "Target Production" non è più un campo del form condiviso da tutto il batch, ma viene ereditato automaticamente per ciascun sottotask dal Task padre scelto per quel comune (un batch può includere più Task/comuni con Target Production diversi). Richiede le Edge Function `jira-create-subtask` (v6) e `jira-list-tasks` (v3) aggiornate (non versionate nel repo).
